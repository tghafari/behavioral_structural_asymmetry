...
dependent_var = ['Landmark_PSE', 'Target_PSE_Laterality', 'Landmark_MS', 'Target_MS_Laterality']
...
for dep in dependent_var:
    print(f'\nRunning models on {dep}')
    # Remove NaNs from dependent variable (but keep rows in the dataset)
    data = data_full.dropna(subset=dep)

    # Step 3: Visualize relationships 
    g = sns.pairplot(data[independent_vars + [dep]])
    g.fig.subplots_adjust(top=0.9, bottom=0.1, left=0.1, right=0.9)
    g.fig.suptitle('Pairplot of Variables', y=0.95, fontsize=16)
    plt.savefig(f'{pairplot_figname}_{dep}.png')
    # plt.show()
    ...
    results_df.to_csv(f'{models_fname}_{dep}.csv')
    ...
    residuals = data[dependent_var] - y_pred
    ...
    plt.savefig(f'{res_figname}_{dep}.png')
    # plt.show()
    ...
    coefficients.plot(kind='bar', color='skyblue', alpha=0.8, edgecolor='black')
    plt.title('Beta Coefficients of the Best Model', fontsize=16)
