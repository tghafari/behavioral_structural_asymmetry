...
# Define file names
sub_code = '103'
base_dir = r'Z:\Projects\Subcortical_Structures\SubStr_and_behavioral_bias\Data'
task_dir = r'target_orientation_detection\Results'
sub_dir = os.path.join('sub-S' + sub_code, r'ses-01\beh')
eyetracking_fpath = os.path.join(base_dir, task_dir, sub_dir, 'e01S' + sub_code) 
eyetracking_asc_file = eyetracking_fpath + '.asc'
eyetracking_compatibel_format = eyetracking_fpath + '.csv'
...
if not os.path.isfile(eyetracking_compatibel_format):
    generateCompatibleFormat(exp_path=eyetracking_asc_file,
                            device="eyelink")
                            stim_list_mode="NA",
...
df = pd.read_csv(eyetracking_compatibel_format)
...
stim = Stimulus(path=eyetracking_asc_file,
               data=df,
