...
output_fpath = op.join(deriv_dir, 'target_orientation')

ms_lateralisation_table =[]
# Load microsaccade info lists
for sub_code in range(1,33):
    output_dir = op.join(output_fpath,'eyetracking', 'sub-S' + str(1000+sub_code))
...
all_subs_fpath = op.join(output_fpath,'lateralisation_indices', 'ms_lateralisation_32subs.csv')
